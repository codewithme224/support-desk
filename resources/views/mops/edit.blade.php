    ...
    <div class="container-fluid py-4">
        <div class="card">
            ...
            <div class="card-body pt-4 p-3">
                <form action="{{ route('mops.update', $mop->id) }}" method="POST" role="form text-left">
                    @csrf
                    @method('PUT')
                    @if($errors->any())
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div class="@error('name')border border-danger rounded-3 @enderror">
                                    <input class="form-control" type="text" placeholder="Title of the MOP" id="name" name="name" value="{{ old('name', $mop->name) }}">
                                        @error('name')
                                            <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                        @enderror
                                </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="@error('description')border border-danger rounded-3 @enderror">
                            <textarea class="form-control" id="description" rows="5" placeholder="Short description about the mop" name="description" data-language="markdown">{{ $mop->description }}</textarea>
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="about">{{ 'Script/Query' }}</label>
                        <div  class="@error('script')border border-danger rounded-3 @enderror">
                            <textarea class="form-control" id="codeInput" rows="10" placeholder="Enter your query of script here" name="script" style="height: 300px;">{{$mop->script}}</textarea>
                        </div>
                    </div>
                    <div class="d-flex justify-content-end">
                        <button type="submit" class="btn bg-gradient-dark btn-md mt-4 mb-4">{{ 'Update MOP' }}</button>
