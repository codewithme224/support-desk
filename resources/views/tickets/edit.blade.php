    ...
    <div class="container-fluid py-4">
        <div class="card">
            <div class="card-header pb-0 px-3">
                <h6 class="mb-0">{{ __('Edit Ticket') }}</h6>
            </div>
            <div class="card-body pt-4 p-3">
                <form action="{{ route('tickets.update', $ticket->id) }}" method="POST" role="form text-left">
                    @csrf
                    @method('PUT')
                    @if($errors->any())
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="issue" class="form-control-label">{{ __('REQUESTS/ISSUE') }}</label>
                                <div class="@error('issue')border border-danger rounded-3 @enderror">
                                    <input class="form-control" type="text" placeholder="State issue reported"  value="{{ $ticket->issue }}" id="issue" name="issue">
                                        @error('issue')
                                            <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                        @enderror
                                </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="status_id" class="form-control-label">{{ __('Status') }}</label>
                    ...
                    <div class="form-group">
                        <label for="about">{{ 'Resolution' }}</label>
                        <div class="@error('resolution')border border-danger rounded-3 @enderror">
                            <textarea class="form-control" id="resolution" rows="5" placeholder="Description of method/steps used to resolve the request " name="resolution">{{ $ticket->resolution }}</textarea>
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="about">{{ 'Method Of Procedure(Select an MOP Or Create one at the MOP section before you procered)' }}</label>
                        <div class="@error('mop')border border-danger rounded-3 @enderror">
                            <select name="mop" id="mop" class="form-control">
                                <option value="">Select MOP</option>
                                @foreach ($mops as $mop)
                                    <option value="{{ $mop->name }}" {{ $mop->id == $selectedMopId ? 'selected' : '' }}>
                                        {{ $mop->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="d-flex justify-content-end">
                        <button type="submit" class="btn bg-gradient-dark btn-md mt-4 mr-4 mb-4">{{ 'Update Ticket' }}</button>
                        &nbsp;&nbsp;
